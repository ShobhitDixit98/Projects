...
#include <ctype.h>	    //for character tests
...
//**********************************************************
// THIS FUNCTION IS TO RUN gotoxy FUNCTION
//**********************************************************
...
//**********************************************************
// CLASS NAME : Menu
// DETAILS : IT CONTROLLS OVER A.L THE FUNCTIONS
//**********************************************************
...
class account
{
    public :
        ...
        void add_bill(int t_billno, int t_itemcode, char *t_itemname, float t_qty, float t_cost, float t_price) ;
    private :
    ...
    while (1)
        {
            ...
            gotoxy(10,23) ;
            cout<<"*************************************************************" ; gotoxy(28,6) ;
            cout <<"S T O R E    M A N A G E M E N T" ;
            gotoxy(28,7) ;
            cout <<"~~~~~~~~~~   ~~~~~~~~~~~~~~~~~~~~" ; gotoxy(30,9) ;
            ...
            cout <<"0: QUIT" ;
            gotoxy(30,20) ;
            cout <<"Enter Your Choice : " ; ch = getch() ;
            if (ch == '1')
                {
                    product p ; p.purchase() ;
                }
            else if (ch == '2')
                {
                    product p ; p.list_of_item() ;
                }
            else if (ch == '3')
                {
                    edit_menu() ;
...
int product :: last_code(void)
{
    ...
    int t=0 ;
    while (file.read((char *) this, sizeof(product))) t = itemcode ;
    file.close() ;
    return t ;
}
...
void product :: list_of_item(void)
{
    ...
    while (file.read((char *) this, sizeof(product)))
        ...
        gotoxy(1,25) ;
        cout <<"Press any key to continue..." ; getche() ;
        file.close () ;
}
...
void product :: add_item(void)
{
    system("color 0b");
    int tcode, valid ;
    char ch, t_itemcost[10], t_itemprice[10] ; tcode = last_code() ;
    tcode++ ;
    do
        {
            system("cls");
    ...
    do
        {
            ...
            if (t_code[0] == '0')
                {
                    if (purchased) a.prepare_bill(t_billno) ;
                    return ;
                }
            if (tcode == 0)
                {
